class Dodgeball extends Phaser.Scene {
  ...
  create() {
    ...
    //emit current players for client side to display
    this.io.emit("currentPlayers_dodge", players)

    //add input handling for connected sockets
  ...
  update() {
    ...
    //check winner function and handle scene stopping
    if(!(getWinnerName() === null)) {
      ...
      const timerInterval= setInterval(() => {
        countdown--;
        //after countdown set alive and invuln
        if(countdown === 0) {
          clearInterval(timerInterval);
...
//called when each player hits the ball, handles removing player
...
//Get winner name to dislay, if no players return null
